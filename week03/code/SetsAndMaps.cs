...
public static class SetsAndMaps
{
    ...
    public static string[] FindPairs(string[] words)
    {
        ...
        foreach (var word in wordSet) // Use the set, not the original array to avoid duplicates
        {
            ...
            var reversed = $"{word[1]}{word[0]}"; // More efficient than Reverse().ToArray()

            // Check if it's a valid pair (not the same word and reverse exists)
            if (word != reversed && wordSet.Contains(reversed) && !processedWords.Contains(reversed))
            {
